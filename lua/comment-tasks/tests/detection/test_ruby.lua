-- Ruby comment detection tests
local M = { language = "Ruby" }
...
function M.run_tests(assert)
    print("  Testing Ruby comment detection...")
    local cleanup = setup_mock_vim({
        "# TODO: Add proper validation to this method",
        "# Current implementation is too permissive",
        "def authenticate(username, password)"
    }, {1, 0}, "ruby")
    
    local detection = require("comment-tasks.core.detection")
    local config = require("comment-tasks.core.config")
    local comment_info = detection.get_comment_info_regex("ruby", config.default_config.languages)
    assert.assert_not_nil(comment_info, "Should detect Ruby comments")
    cleanup()
    print("  ✅ Ruby detection tests completed")
end
return M