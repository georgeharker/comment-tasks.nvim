-- PHP comment detection tests
local M = { language = "PHP" }
...
function M.run_tests(assert)
    print("  Testing PHP comment detection...")
    ...
    local comment_info = detection.get_comment_info_regex("php", config.default_config.languages)
    assert.assert_not_nil(comment_info, "Should detect PHP comments")
    cleanup()
    print("  ✅ PHP detection tests completed")
end
return M